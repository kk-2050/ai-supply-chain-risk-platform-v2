...
| Error handling | Partially Confirmed | `Parse AI Risk Output` validates the parsed AI response (required fields, non-empty text after trimming, integer 1–5 risk scores, and a finite 0–100 `confidence_score`) before scoring or routing. Two distinct sanitized fallback outcomes exist, neither retaining invalid AI-generated content: `processing_status: 'parse_error'` when the response fails to parse as JSON, and `processing_status: 'validation_error'` when it parses but fails field validation. Both fallback records preserve the trusted `source_url` and, when available as a non-empty string, the original article title from `supplierData.risk_title`; `error_message` and `ai_summary` are fixed, content-free messages, and `recommended_action` reads "Review the source event and workflow execution before reprocessing." Both fallback types use forced safe scores (`overall_risk_score: 1.00`, `alert_level: 'low'`), excluding them from both Critical Alert and Operational Summary routing under the existing thresholds. No automatic retry exists in this phase. Because the record's `source_url` is written to the same sheet used for deduplication, the source event is preserved for investigation but not currently automatically reprocessable. Handling for other failure types (RSS fetch, Sheets writes, email delivery) remains unconfirmed |
...
| Validation | Confirmed | `Parse AI Risk Output` validates that the parsed AI response is a non-null JSON object, not an array, and that all eight required fields (`risk_title`, `risk_category`, `severity_score`, `operational_impact_score`, `urgency_score`, `confidence_score`, `ai_summary`, `recommended_action`) pass validation before numeric coercion, scoring, routing, or a success record is built. `risk_title`, `risk_category`, `ai_summary`, and `recommended_action` must be non-empty strings after trimming; `severity_score`, `operational_impact_score`, and `urgency_score` must be true JSON integers from 1–5; `confidence_score` must be a true finite JSON number from 0–100 (decimals allowed). Numeric strings are rejected, not coerced; an unrecognized but non-empty `risk_category` still normalizes to `Other`. This checks structure, required fields, type, and range only — it does not verify semantic accuracy, factual correctness, evidence support, or hallucinations. Invalid parsed output becomes a sanitized `validation_error` record (see Error handling row); no retry is implemented in Phase 1. Separately, the Operational Summary prompt instructs the model to use only supplied data and never invent suppliers, risks, scores, or URLs — a prompt-level guardrail, not code-level validation of the output |
